...
## Exploring  
We can also observe what our trace plots would look like under less ideal conditions. For example, what would happen if we chose a starting value that was very far away from our true $a$ and $b$? In this trial, we set $a$ and $b$ to each start at 100.
...
On the other hand, what happens if we pick an extremely small starting point?
```{r, include=FALSE}
set.seed(1)
# true data distribution
N <- 100  # 100 Xi observations
...
In both extremes, our Gibbs Sampler has a good mixing rate and fluctuates around the true values for $a$ and $b$ after approximately 100 trials.      
...
$$a{\sim} \text{Gamma}(1,5)$$ 
$$b{\sim} \text{Gamma}(1,5)$$
This would change our conditional distributionns to be:
...
With a different prior, we can see that our Gibbs Sampler isn't as good. While it manages to converge and fluctuate around the true $a$ value, it hasn't begun fluctuating around the true $b$ value, even after 1000 trials.  
...
2. A good Gibbs Sampler will have a high mixing rate and a low burn-in period.  

3. While it is not vital to choose a starting point near the true value, it is important that you have priors that reasonably match the data that you are trying to model. Otherwise, a Gibbs Sampling simulation may not find the correct values.
